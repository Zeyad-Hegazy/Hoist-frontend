...
const Part9 = ({ onUpdate }) => {
	const textRefcablesPanels = useRef({ comment: "" });
	const textReflighting = useRef({ comment: "" });

	const [formData, setFormData] = useState({
		cablesPanels: { condition: "" },
		lighting: { condition: "" },
	});
	...
	const updateParent = useCallback(() => {
		onUpdate("part9", {
			cablesPanels: {
				...formData.cablesPanels,
				comment: textRefcablesPanels.current.comment,
			},
			lighting: {
				...formData.lighting,
				comment: textReflighting.current.comment,
			},
		});
	}, [onUpdate, formData]);
	...
	const handleCommentChange = (key, value) => {
		if (key === "cablesPanels") {
			textRefcablesPanels.current.comment = value;
		} else if (key === "lighting") {
			textReflighting.current.comment = value;
		}
	};
	...
	return (
		<Accordion style={{ marginBlock: "10px" }}>
			<AccordionSummary expandIcon={<IconExpand />}>
				<Typography>Part 9</Typography>
			</AccordionSummary>
			<AccordionDetails>
				{[
					{ label: "Cables Panels", key: "cablesPanels" },
